...
function mc_theme_mc_icon($atts)
{
	...
	$output = '';
	if($type=="features" && ($style=="hexagon" || $style=="circle"))
		$output .= '<' . ($url!="" ? 'a' : 'span') . ($url!="" ? ' href="' . esc_attr($url) . '"' . ($url_target=="new_window" ? ' target="_blank"' : '') : '') . ' class="icon-single ' . ($style=="circle" ? 'circle' : 'hexagon') . ($hexagon_size=="small" ? ' small' : '') . ($hexagon_style=="light" ? ' style-light' : '') . ($class!="" ? ' ' . esc_attr($class) : '') . ($top_margin!="none" ? ' ' . esc_attr($top_margin) : '') . '"' . ($title!="" ? ' title="' . esc_attr($title) . '"' : '') . '>';
	$output .= '<' . ($url!="" && ($type!="features" || ($type=="features" && $style!="hexagon" && $style!="circle")) ? 'a' : 'span') . ($url!="" && ($type!="features" || ($type=="features" && $style!="hexagon" && $style!="circle")) ? ' href="' . esc_attr($url) . '"' . ($url_target=="new_window" && ($type!="features" || ($type=="features" && $style!="hexagon" && $style!="circle")) ? ' target="_blank"' : '') : '') . ' class="' . ($type!="features" || ($type=="features" && $style!="hexagon" && $style!="circle") ? 'icon-single mc-icon ' : '') . esc_attr($type) . '-' . ($type=="features" ? esc_attr($icon_feature) : ($type=="social" ? esc_attr($icon_social) : esc_attr($icon_template))) . ($class!="" && ($type!="features" || ($type=="features" && $style!="hexagon" && $style!="circle")) ? ' ' . esc_attr($class) : '') . ($top_margin!="none" && ($type!="features" || ($type=="features" && $style!="hexagon" && $style!="circle")) ? ' ' . esc_attr($top_margin) : '') . '"' . ($title!="" && ($type!="features" || ($type=="features" && $style!="hexagon" && $style!="circle")) ? ' title="' . esc_attr($title) . '"' : '') . '></' . ($url!="" && ($type!="features" || ($type=="features" && $style!="hexagon" && $style!="circle")) ? 'a' : 'span') . '>';
	if($type=="features" && ($style=="hexagon" || $style=="circle"))
		$output .= '</' . ($url!="" ? 'a' : 'span') . '>';
	return $output;
}
...
vc_map( array(
	...
	"params" => array(
		...
		array(
			...
			"param_name" => "style",
			"value" => array(__("Simple", 'medicenter') => "simple", __("Hexagon", 'medicenter') => "hexagon", __("Circle", 'medicenter') => "circle"),
		...
		array(
			...
			"dependency" => Array('element' => "style", 'value' => array('hexagon', 'circle'))
		),
		array(
			"type" => "dropdown",
			"class" => "",
