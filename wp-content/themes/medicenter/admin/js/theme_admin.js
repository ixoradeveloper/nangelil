jQuery(document).ready(function($){
	//upload
	$(document.body).on("click", "[name='medicenter_upload_button']", function(){
		var self = $(this);
	...
	$("#theme-options-panel").on("submit", function(event){
		...
		$("#theme_options_info").html("").css("display", "none");
		$.ajax({
				url: ajaxurl,
				type: 'post',
				dataType: 'json',
				...
				success: function(json){
					...
					{
						$("#theme_options_info").html(json.error).css("display", "block");
					}
					else
					{
	...
	$("#import_dummy").on("click", function(event){
		...
		var demo = $("#import_dummy_demo").val();
		if(demo=="pregnancy_home" || demo=="ophthalmologist_home" || demo=="veterinary_home" || demo=="dentist_home")
		{
			$.ajax({
					url: ajaxurl,
					type: 'post',
					data: "action=ql_importer_import_dummy2&themename=" + config.themename + "&demo=" + demo,
					success: function(jsonSecond){
						...
						$("#dummy_content_info").html(jsonSecond.info);
					},
