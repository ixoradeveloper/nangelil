...
! function(window) {
    ...
    function() {
        ...
        function scrollToElement() {
            var iframe, offsetPosition, model = window.vc.latestAddedElement;
            model && model.view && model.view.el && (model = model.view.el.getBoundingClientRect().top, iframe = document.getElementById("vc_inline-frame"), offsetPosition = model + (window.scrollY || document.documentElement.scrollTop) - 250, (iframe ? (offsetPosition = model + ((model = iframe.contentWindow).scrollY || model.document.documentElement.scrollTop) - 250, model) : window).scrollTo({
                top: offsetPosition,
                behavior: "smooth"
            }))
        }
        ...
        window.vc.element_start_index = 0, window.vc.AddElementUIPanelBackendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).extend({
            ...
            createElement: function(e) {
                ...
                e && e.preventDefault && e.preventDefault(), tag = (e = $(e.currentTarget)).data("tag"), row_params = {}, column_params = {
                    ...
                    order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.getNextOrder(),
                    root_id: this.model.get("root_id")
                }, preset && presetType === tag && (options.preset = preset), vc.shortcodes.create(options)), this.model = model, window.vc.latestAddedElement = model, column_params = !(_.isBoolean(vc.getMapped(tag).show_settings_on_create) && !1 === vc.getMapped(tag).show_settings_on_create), this.model.get("shortcode"), this.hide(), column_params && this.showEditForm()
            },
            getFirstPositionIndex: function() {
            ...
            createElement: function(e) {
                ...
                _.isString(vc.getMapped(tag).default_content) && vc.getMapped(tag).default_content.length && (row_params = this.builder.parse({}, window.vc.getMapped(tag).default_content, this.builder.last().toJSON()), _.each(row_params, function(object) {
                    object.default_content = !0, this.builder.create(object)
                }, this)), this.model = this.builder.last(), window.vc.latestAddedElement = this.model, column_params = !(_.isBoolean(vc.getMapped(tag).show_settings_on_create) && !1 === vc.getMapped(tag).show_settings_on_create), this.hide(), column_params && this.showEditForm(), this.builder.render(null, this.model)
            }
        })
    }(window.jQuery),
    function($) {
        ...
        window.vc.EditElementPanelView = vc.PanelView.vcExtendUI(vc.HelperAjax).vcExtendUI(vc.ExtendPresets).vcExtendUI(vc.ExtendTemplates).vcExtendUI(vc.HelperPrompts).extend({
            ...
            formRender: function() {
                ...
                this.$el.find(".vc_edit-form-tab").on("input change", function(e) {
                    clearTimeout(typingTimer), typingTimer = setTimeout(function() {
                        _this.save(e)
                    }, 500)
                })
            },
            ...
            save: function(e) {
                vc.saveInProcess = !0;
                e = e && e.target && e.target.classList.contains("textarea_html");
                if (this.panelInit && this.model) {
                    ...
                    this.model.save({
                        params: mergedParams
                    }), this.showMessage(window.sprintf(window.i18nLocale.inline_element_saved, vc.getMapped(shortcode).name), "success"), window.vc_auto_save || window.vc.frame_window || this.hide(), this.trigger("save"), vc.saveInProcess = !1
                }
            },
            show: function() {
            ...
            hide: function(e) {
                e && e.preventDefault && e.preventDefault(), vc.saveInProcess && this.$el.addClass("visually-hidden");
                e = vc.saveInProcess ? 500 : 0;
                setTimeout(function() {
                    this.checkAjax(), this.ajax = !1, this.model && (this.model = null), vc.active_panel = !1, this.currentModelParams = !1, this._killEditor(), this.$el.removeClass("vc_active visually-hidden"), this.$el.find(".vc_properties-list").removeClass("vc_with-tabs").css("margin-top", "auto"), this.$content.empty(), this.trigger("hide")
                }.bind(this), e)
            },
            setTitle: function() {
    ...
    function($) {
        ...
        window.init_textarea_html = function($element) {
            var textfield_id, $content_holder, isModified, $wp_link = $("#wp-link");
            ...
            try {
                _.isUndefined(tinyMCEPreInit.qtInit[textfield_id]) && (window.tinyMCEPreInit.qtInit[textfield_id] = _.extend({}, window.tinyMCEPreInit.qtInit[window.wpActiveEditor], {
                    id: textfield_id
                })), window.tinyMCEPreInit && window.tinyMCEPreInit.mceInit[window.wpActiveEditor] && (isModified = !1, window.tinyMCEPreInit.mceInit[textfield_id] = _.extend({}, window.tinyMCEPreInit.mceInit[window.wpActiveEditor], {
                    resize: "vertical",
                    height: 200,
                    id: textfield_id,
                    setup: function(ed) {
                        ...
                        }), window.vc_auto_save && (debouncedSave = _.debounce(function() {
                            isModified = !0, vc.edit_element_block_view.save()
                        }, 500), ed.on("keyup", debouncedSave), ed.on("blur", function() {
                            isModified && (isModified = !1, vc.edit_element_block_view.save())
                        }), ed.on("ExecCommand", debouncedSave))) : ed.onInit.add(function() {
                            window.wpActiveEditor = textfield_id
                        })
                    }
        ...
        var GoogleFonts = Backbone.View.extend({
            ...
            VC_ParamGroup = Backbone.View.extend({
                ...
                initialize: function(data) {
                    ...
                    this.$ul = this.$el.find("> .edit_form_line > .vc_param_group-list"), $elParam = $("> .wpb_vc_row", this.$ul), this.initializer = new Vc_ParamInitializer({
                        ...
                        stop: function() {
                            $(this).closest(".vc_edit-form-tab").trigger("change")
                        }
                    })
                },
                addNew: function(ev) {
            ...
            getControlsHTML = (window.i18nLocale, vc.edit_form_callbacks = [], vc.atts = {
                ...
                parse: function(param) {
                    var new_value, param = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]");
                    try {
                        new_value = window.vc.htmlHelpers.fixUnclosedTags(param.val())
                    } catch (e) {
                        new_value = param.val(), console.error("Failed to execute window.vc.htmlHelpers.fixUnclosedTags function: ", e)
                    }
                    ...
                    $(".vc_color-control", $field).each(function() {
                        ...
                        value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/) && (alpha_val = 100 * parseFloat(value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/)[1])), $control.wpColorPicker({
                            clear: function(event) {
                                $alpha.val(100), $alpha_output.val("100%"), event && event.originalEvent && event.originalEvent.isTrusted && "click" === event.type && $(this).trigger("change")
                            },
                    ...
                    $('[data-vc-action="open"]', _this.content).on("click", function(e) {
                        ...
                        _this.gutenbergParamObj = new GutenbergParam({
                            onSetValue: function(value) {
                                $field_input.val(value), $field_input.trigger("change")
                            },
        ...
        vc.saved_custom_css = !1, vc.createPreLoader = function() {
            ...
            buildRelevance: function() {
                vc.shortcode_relevance = {}, _.map(vc.map, function(object) {
                    _.isObject(object.as_parent) && _.isString(object.as_parent.only) && (vc.shortcode_relevance["parent_only_" + object.base] = object.as_parent.only.replace(/\s/g, "").split(",")), _.isObject(object.as_parent) && _.isString(object.as_parent.except) && (vc.shortcode_relevance["parent_except_" + object.base] = object.as_parent.except.replace(/\s/g, "").split(",")), _.isObject(object.as_child) && _.isString(object.as_child.only) && (vc.shortcode_relevance["child_only_" + object.base] = object.as_child.only.replace(/\s/g, "").split(",")), _.isObject(object.as_child) && _.isString(object.as_child.except) && (vc.shortcode_relevance["child_except_" + object.base] = object.as_child.except.replace(/\s/g, "").split(","))
    ...
    function($) {
        ...
        $promoModal.on("click", function(e) {
            $(e.target).closest('[data-vc-ui-element="button-close"]').length && $promoModal.removeClass("vc_active")
        })
    }(window.jQuery);