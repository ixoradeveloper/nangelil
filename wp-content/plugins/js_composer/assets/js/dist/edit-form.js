    ...
    function($) {
        ...
        media = wp.media, preloader_url = ajaxurl.replace(/admin\-ajax\.php/, "images/wpspin_light.gif"), media.controller.VcCssSingleImage = media.controller.VcSingleImage.extend({
            ...
            render: function(value) {
                return this.attrs = {}, this.$simplify = this.$el.find(".vc_simplify"), this.setPlaceholders(), _.isString(value) && this.parse(value), this
            },
                ...
                ! function(selection, obj) {
                    ...
                    ids = [], $(".media-modal").addClass("processing-media"), selection.each(function(model) {
                        ...
                        function(ids, callback) {
                            ...
                            obj._css_editor.$el.find(".vc_background-image .vc_image").html(template(newAttachments[0])), $(".media-modal").removeClass("processing-media"), obj._css_editor.$el.closest(".vc_edit-form-tab").trigger("change")
                        })
            ...
            removeImage: function(e) {
                ...
                e && e.preventDefault && e.preventDefault(), $control.parent().remove(), this.$el.closest(".vc_edit-form-tab").trigger("change")
            },
            ...
            setPlaceholders: function() {
                try {
                    var key, shortcode = this.$el.closest("#vc_ui-panel-edit-element").attr("data-vc-shortcode"),
                        valuesObject = window.vc.map[shortcode].params.find(function(param) {
                            return "css_editor" === param.type
                        }).value;
                    for (key in valuesObject) {
                        var tagName, isColorPicker, name = key.replace(/\-+/g, "_"),
                            $element = this.$el.find("[name=" + name + "]");
                        $element.length && (tagName = $element[0].tagName, $element.attr("placeholder", valuesObject[key]), isColorPicker = $element.hasClass("vc_color-control"), "SELECT" === tagName || isColorPicker) && ($element.val(valuesObject[key]), isColorPicker) && $element.trigger("change")
                    }
                } catch (e) {
            ...
            getBackground: function() {
                var color = this.getBackgroundColor(),
                    ...
                    colorPlaceholder = this.$el.find("[name=background_color]").attr("placeholder"),
                    useColor = color;
                colorPlaceholder && color.toLowerCase() !== colorPlaceholder.toLowerCase() && (useColor = color), color && image ? this.attrs.background = color + " url(" + image + ")" : useColor ? this.attrs["background-color"] = useColor : image && (this.attrs["background-image"] = "url(" + image + ")"), style.match(/repeat/) ? (this.attrs["background-position"] = "0 0", this.attrs["background-repeat"] = style) : style.match(/cover|contain/) && (this.attrs["background-position"] = "center", this.attrs["background-repeat"] = "no-repeat", this.attrs["background-size"] = style), color.match(/^rgba/) && (this.attrs["*background-color"] = color.replace(/\s+/, "").replace(/(rgb)a\((\d+)\,(\d+),(\d+),[^\)]+\)/, "$1($2,$3,$4)"))
            },
            getBorder: function() {
                var style = this.$el.find("[name=border_style]").val(),
                    stylePlaceholder = this.$el.find("[name=border_style]").attr("placeholder"),
                    radius = this.$el.find("[name=border_radius]").val(),
                    radiusPlaceholder = this.$el.find("[name=border_radius]").attr("placeholder"),
                    color = this.$el.find("[name=border_color]").val(),
                    colorPlaceholder = this.$el.find("[name=border_color]").attr("placeholder");
                this.attrs["border-width"] && this.attrs["border-width"].match(/^\d+\S+$/) ? (this.attrs.border = this.attrs["border-width"] + " " + (style || "initial") + " " + color, this.attrs["border-width"] = void 0, radius && radiusPlaceholder && radius !== radiusPlaceholder && (this.attrs["border-radius"] = radius)) : (_.each(["left", "right", "top", "bottom"], function(side) {
                    this.attrs["border-" + side + "-width"] && style && (stylePlaceholder && style !== stylePlaceholder || !stylePlaceholder) && (this.attrs["border-" + side + "-style"] = style)
                }, this), radius && (radiusPlaceholder && radius !== radiusPlaceholder || !radiusPlaceholder) && (this.attrs["border-radius"] = radius), color && (colorPlaceholder && color.toLowerCase() !== colorPlaceholder.toLowerCase() || !colorPlaceholder) && (this.attrs["border-color"] = color))
            },
            getFields: function(type) {
                var data = [];
                if (this.simplify) return this.getSimplifiedField(type);
                _.each(this.positions, function(pos) {
                    var val = this.$el.find("[data-name=" + type + "-" + pos + "]").val();
                    (val = (val = val.replace(/\s+/, "")).match(/^-?\d*(\.\d+){0,1}(%|in|cm|mm|em|rem|ex|pt|pc|px|vw|vh|vmin|vmax)$/) ? val : isNaN(parseFloat(val)) ? "" : parseFloat(val) + "px") && val.length && data.push({
                        name: pos,
                        val: val
                    })
